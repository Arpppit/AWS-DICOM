...
cursor.execute('CREATE TABLE IF NOT EXISTS IMAGES( ID INTEGER PRIMARY KEY AUTOINCREMENT,PATIENTID STR NOT NULL, SERIESINSTANCEUID STR NOT NULL,SOPINSTANCEUID STR NOT NULL UNIQUE, FILENAME STR NOT NULL UNIQUE);')
cursor.execute('CREATE TABLE IF NOT EXISTS JOBS( ID INTEGER PRIMARY KEY AUTOINCREMENT,PID STR NOT NULL , SOPINSTANCEUID STR NOT NULL UNIQUE, PATH STR NOT NULL UNIQUE, SENT BOOLEAN);')
...
def write_to_db(pid, studyid, seriesid,sopid,frameid,path, fname):
    ...
    cursor.execute(f'INSERT OR IGNORE INTO IMAGES( PATIENTID,SERIESINSTANCEUID, SOPINSTANCEUID ,FILENAME) VALUES ("{pid}" , "{seriesid}", "{sopid}", "{path}")' )
    db.commit()
...
def schedule(path,sopid,pid):
  global cursor
  global db
  cursor.execute(f'INSERT OR IGNORE INTO JOBS(PID,SOPINSTANCEUID, PATH, SENT) VALUES ("{pid}","{sopid}", "{path}", {False})')
...
def handle_store(event, storage_dir):
    ...
    # if event.dataset.Modality == 'CT' and not(event.dataset.SeriesDescription =='AI-Rad Companion Pulmonary Lesion Thumbnails'):
    #     series_folder_ct = study_folder + '/' + seriesid
    series_folder = study_folder + '/' + seriesid
    os.makedirs(series_folder,exist_ok=True)
   
    
    
    if event.dataset.Modality =='SR':
        storage_dir = series_folder + '/SR' 
        ...
        try:
            if event.dataset.SeriesDescription =='AI-Rad Companion Pulmonary Lesion Thumbnails':
              ...
              storage_dir = series_folder+ '/Thumbnails' # image-series data
              thumb_f = storage_dir
              ...
              storage_dir = series_folder + '/CT' # image-series data
              os.makedirs(storage_dir, exist_ok=True)
              fname = os.path.join(storage_dir+'/', event.request.AffectedSOPInstanceUID)
              ...
              os.makedirs(storage_dir, exist_ok=True)
              fname = os.path.join(storage_dir+'/', event.request.AffectedSOPInstanceUID)
              logging.error(f'{e} occured. Recieved {event.dataset.Modality} Maybe Dataset is corrupted or not part of SR CT or thumbnails')
    ...
    if event.dataset.Modality =='SR':
      schedule(series_folder,sopid, pid)
